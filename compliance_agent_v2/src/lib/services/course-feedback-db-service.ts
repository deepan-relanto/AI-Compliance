...
import type { FeedbackRow } from "@/lib/services/feedback-db-service";

type Sql = ReturnType<typeof getSql>;

export async function listCourseFeedback(sql: Sql): Promise<FeedbackRow[]> {
  const rows = await sql`
    ...
    FROM course_feedback_entries fe
...
export async function createCourseFeedback(
  ...
  await sql`
    INSERT INTO course_feedback_entries (id, user_id, user_name, assessment_id, assessment_name, feedback_text)
    VALUES (
      ${params.id},
      ${params.userId},
      ${params.userName},
