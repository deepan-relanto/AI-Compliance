/**
 * Replace the video asset for a course module step and persist to Neon.
 * Usage: node scripts/db-replace-course-video.mjs <path-to-mp4> [moduleId]
 */
...
if (!mp4Path || !fs.existsSync(mp4Path)) {
  console.error("Usage: node scripts/db-replace-course-video.mjs <path-to-mp4> [moduleId]");
  process.exit(1);
}

const buffer = fs.readFileSync(mp4Path);
const originalName = path.basename(mp4Path);
...
if (!moduleId) {
  const rows = await sql`
    ...
    WHERE s.step_type = 'video'
    ORDER BY s.module_id
    LIMIT 5
  `;
  if (rows.length === 0) {
    console.error("No course video steps found.");
    process.exit(1);
  }
  moduleId = rows[0].module_id;
  if (rows.length > 1) {
    console.log("Multiple course modules with video:", rows.map((r) => r.module_id));
  }
}

const stepRows = await sql`
  SELECT id, config FROM course_module_steps
  WHERE module_id = ${moduleId} AND step_type = 'video'
  LIMIT 1
`;
if (stepRows.length === 0) {
  console.error(`No video step for module ${moduleId}`);
  process.exit(1);
}
...
console.log(`✅ Video updated for module ${moduleId}`);
