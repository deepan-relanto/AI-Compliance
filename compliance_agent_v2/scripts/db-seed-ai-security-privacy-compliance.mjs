/**
 * Create / refresh the full AI Security, Privacy & Compliance course bundle in Neon.
 *
 * Assets:
 *  - lesson HTML, scenarios HTML, mindmap HTML
 *  - video MP4 (Enterprise_AI_Security.mp4)
 *  - optional infographic PNG (skipped if missing)
 *  - 20-question quiz bank
 *
 * Usage:
 *   node scripts/db-seed-ai-security-privacy-compliance.mjs
 *   node scripts/db-seed-ai-security-privacy-compliance.mjs --reuse <existingModuleId>
 *   node scripts/db-seed-ai-security-privacy-compliance.mjs --no-batches
 */
...
const TITLE = "AI Security, Privacy & Compliance";
const DESCRIPTION =
  "Protect company and personal data when using AI — classification, safe prompting, shadow AI, compliance, and incident response.";
...
const MINDMAP_SRC = path.join(
  ...
  "content-kit",
  "mindmap-html",
  "mindmap-ai-security-privacy-compliance.html",
);
...
const VIDEO_CANDIDATES = [
  path.join(root, "content-kit", "video", "Enterprise_AI_Security.mp4"),
  path.join(
    process.env.USERPROFILE || "",
    "Downloads",
    "Enterprise_AI_Security.mp4",
  ),
];
...
try {
  console.log("\n=== Seed AI Security, Privacy & Compliance ===\n");
  ...
  const lesson = await storeMediaAsset({
    srcPath: LESSON_SRC,
    originalName: "relanto_ai_security_privacy_compliance_interactive.html",
    mimeType: "text/html",
    ext: ".html",
  });
  ...
  const scenarios = await storeMediaAsset({
    ...
    originalName: "relanto_ai_security_scenarios_interactive.html",
    mimeType: "text/html",
    ext: ".html",
  });
  ...
  const video = await storeMediaAsset({
    srcPath: videoPath,
    originalName: "Enterprise_AI_Security.mp4",
    mimeType: "video/mp4",
    ext: ".mp4",
  });
  ...
  const mindmap = await storeMediaAsset({
    srcPath: MINDMAP_SRC,
    originalName: "mindmap-ai-security-privacy-compliance.html",
    mimeType: "text/html",
    ext: ".html",
  });
  ...
  if (hasInfographic) {
    const infographic = await storeMediaAsset({
      ...
      originalName: "AI_Security_Privacy_Compliance.png",
      mimeType: "image/png",
      ext: ".png",
    });
    ...
    console.log("  ⏭  infographic → skipped (no Module 3 PNG yet)");
  }
