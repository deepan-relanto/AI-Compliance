/**
 * Create / refresh the Prompt Engineering Essentials (Module 4) course bundle in Neon.
 *
 * Assets:
 *  - lesson HTML, scenarios HTML, mindmap HTML
 *  - optional video MP4 (skipped if missing — add later)
 *  - optional infographic PNG (skipped if missing — add later)
 *  - 20-question quiz bank
 *
 * Usage:
 *   node scripts/db-seed-prompt-engineering-essentials.mjs
 *   node scripts/db-seed-prompt-engineering-essentials.mjs --reuse <existingModuleId>
 *   node scripts/db-seed-prompt-engineering-essentials.mjs --no-batches
 */
...
const TITLE = "Prompt Engineering Essentials";
const DESCRIPTION =
  "Give clear, complete instructions to AI — five building blocks, core techniques, templates, refinement, and safe workplace prompting.";
...
const MINDMAP_SRC = path.join(
  ...
  "content-kit",
  "mindmap-html",
  "mindmap-prompt-engineering-essentials.html",
);
const INFOGRAPHIC_SRC = path.join(
  root,
  "content-kit",
  "infographic",
  "Prompt_Engineering_Essentials.png",
);
const VIDEO_CANDIDATES = [
  path.join(root, "content-kit", "video", "Prompt_Engineering_Essentials.mp4"),
  path.join(
    process.env.USERPROFILE || "",
    "Downloads",
    "Prompt_Engineering_Essentials.mp4",
  ),
  path.join(
    process.env.USERPROFILE || "",
    "Downloads",
    "Topic4_Prompt_Engineering_Essentials.mp4",
  ),
];
const QUIZ_SRC = path.join(
  ...
  "content-kit",
  "interactive-html",
  "prompt_engineering_essentials_questions.json",
);
...
try {
  console.log("\n=== Seed Prompt Engineering Essentials (Module 4) ===\n");
  ...
  const hasInfographic = fs.existsSync(INFOGRAPHIC_SRC);
  ...
  const lesson = await storeMediaAsset({
    srcPath: LESSON_SRC,
    originalName: "relanto_prompt_engineering_essentials_interactive.html",
    mimeType: "text/html",
    ext: ".html",
  });
  ...
  const scenarios = await storeMediaAsset({
    ...
    originalName: "relanto_prompt_engineering_scenarios_interactive.html",
    mimeType: "text/html",
    ext: ".html",
  });
  ...
  if (videoPath) {
    ...
    console.log("  ⏭  video → skipped (drop Prompt_Engineering_Essentials.mp4 into content-kit/video later)");
  }

  const mindmap = await storeMediaAsset({
    srcPath: MINDMAP_SRC,
    originalName: "mindmap-prompt-engineering-essentials.html",
    mimeType: "text/html",
    ext: ".html",
  });
  ...
  if (hasInfographic) {
    const infographic = await storeMediaAsset({
      ...
      originalName: "Prompt_Engineering_Essentials.png",
      mimeType: "image/png",
      ext: ".png",
    });
    ...
    console.log("  ⏭  infographic → skipped (no Module 4 PNG yet)");
  }
